...
export const mountMouseButtonTest = (root: HTMLElement): MouseButtonController => {
  ...
  const render = (): void => {
    renderStandardMouseVisual(visual, {
      heldButtons: [0, 1, 2, 3, 4].map((button) => state.heldButtons.has(button as 0 | 1 | 2 | 3 | 4)) as [boolean, boolean, boolean, boolean, boolean],
      wheelDirection: null,
    ...
    rows.forEach((row) => {
      ...
      if (detected) detected.textContent = state.detectedButtons.has(button) ? 'Detected' : 'Not yet detected';
    });
    status.textContent = state.detectedButtons.size > 0 ? `${state.detectedButtons.size} button roles detected` : 'Listening for button input';
  };

  const unsubscribe = service.subscribe((event) => {
    if (destroyed || event.type === 'poll-samples') return;
    state = reduceMouseButtonState(state, event);
    render();
    if (event.type === 'buttondown') summary.textContent = `Button role ${event.button} detected. ${state.pressCounts[event.button]} presses observed.`;
  });

  const handleReset = (): void => { state = createMouseButtonState(); summary.textContent = 'Mouse Button Test is listening.'; render(); };
  reset.addEventListener('click', handleReset);
  if (!service.start()) status.textContent = 'Mouse input unavailable';
  render();

  return {
    start: () => { if (!destroyed) { service.start(); render(); } },
    stop: () => { if (!destroyed) { service.stop(); state = reduceMouseButtonState(state, { type: 'clear', reason: 'blur' }); render(); } },
