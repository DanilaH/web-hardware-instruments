...
import { getOledBurnInRuntimeMessages } from './oled-burn-in';

const EXPECTED_PATTERN_COUNT = 8;

describe('OLED burn-in runtime localization', () => {
  it('provides one non-empty label per OLED pattern in every implemented locale', () => {
  ...
  it('keeps every primary OLED control string non-empty in every implemented locale', () => {
    for (const locale of implementedContentLocales) {
      const messages = getOledBurnInRuntimeMessages(locale);
