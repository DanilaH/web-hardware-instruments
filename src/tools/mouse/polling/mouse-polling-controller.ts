...
const sourceLabel = (source: MousePollingSource | null): string =>
  source === 'raw-pointer'
    ? 'High-frequency pointer updates'
    : source === 'coalesced-pointer'
      ? 'Coalesced pointer samples'
      : source === 'basic-pointer'
...
export const mountMousePollingTest = (root: HTMLElement): MousePollingController => {
  ...
  const cancel = (): void => {
    if (!active) return;
    detach();
    status.textContent = 'Test cancelled — keep this page visible and try again.';
    rate.textContent = '—';
  };
  ...
  const finish = (): void => {
    ...
    if (!result) {
      status.textContent = 'Not enough movement — try again.';
      rate.textContent = '—';
      return;
    }
  ...
  const handleStart = (): void => {
    ...
    unsubscribe = service.subscribe((event) => {
      if (event.type === 'poll-samples' && active) {
        timestamps.push(...event.timestamps);
      } else if (event.type === 'clear' && active) {
        cancel();
      }
    });
    ...
    if (!service.start()) {
      unsubscribe();
      status.textContent = 'Pointer sampling unavailable';
    ...
    start.disabled = true;
    rate.textContent = '—';
    status.textContent = 'Measuring for 2 seconds…';
