import { createMouseInputService, type MouseSemanticButton } from '../../../browser/mouse-input-service';
...
export interface DoubleClickController { start(): void; stop(): void; destroy(): void; }
type Messages = ToolRuntimeMessages<'doubleClick'>;
const requireElement = <T extends Element>(root: ParentNode, selector: string): T => {
  const element = root.querySelector<T>(selector);
  if (!element) throw new Error(`Double Click Test is missing ${selector}`);
  return element;
};
const formatGap = (value: number | null): string => value === null ? '—' : `${Math.round(value)} ms`;

export const mountDoubleClickTest = (root: HTMLElement, messages: Messages): DoubleClickController => {
  ...
  const getStatusText = (): string => {
    if (!available) return messages.unavailable;
    if (state.rapidRepeatEvents > 0) return messages.rapidObserved;
    if (state.totalPresses === 0) return messages.listening;
    if (state.shortestGapMs === null) return messages.waitingAnother;
    return messages.noRapid;
  };
  const render = (): void => {
    ...
    renderStandardMouseVisual(visual, { heldButtons: [0, 1, 2, 3, 4].map((button) => held.has(button as MouseSemanticButton)) as [boolean, boolean, boolean, boolean, boolean], wheelDirection: null, movementDetected: false });
  };
  const unsubscribe = service.subscribe((event) => {
    if (destroyed || event.type === 'poll-samples') return;
    if (event.type === 'buttondown') { held.add(event.button); state = observeButtonPress(state, event.button, event.timestamp); render(); }
    else if (event.type === 'buttonup') { held.delete(event.button); render(); }
    else if (event.type === 'clear') { held = new Set(); render(); }
  });
  const handleReset = (): void => { state = createDoubleClickState(); held = new Set(); render(); };
  reset.addEventListener('click', handleReset);
  available = service.start();
  render();
  return {
    start: () => { if (!destroyed) { available = service.start(); render(); } },
    stop: () => { if (!destroyed) { service.stop(); held = new Set(); render(); } },
